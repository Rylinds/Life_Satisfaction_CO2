...
def build_fig1():
    ...
    fig = px.scatter(
        ...
        hover_data={"co2_per_capita": ":.2f", "life_satisfaction": ":.2f",
                    "region": True, "Year": False, "co2_key": False},
        labels={"co2_per_capita": "CO₂ per Capita (tonnes/person)",
                "life_satisfaction": "Life Satisfaction (0–10)", "region": "Region"},
        title="<b>CO₂ Emissions vs. Life Satisfaction</b>  (2014–2024)",
    ...
    _gt = dict(x=x_smooth_v1, y=y_smooth_v1, mode="lines",
               line=dict(color=SLATE, width=2.5, dash="dot"),
               hovertemplate="CO₂ = %{x:.2f}<br>Gaussian fit = %{y:.2f}<extra>Gaussian fit</extra>")
    fig.add_trace(go.Scatter(**_gt, name=f"Gaussian fit  (R²={r2_gauss:.2f})", showlegend=True))
    for frame in fig.frames:
        frame.data = tuple(frame.data) + (
            go.Scatter(**_gt, name="Gaussian fit", showlegend=False),
        )
    peak_y = float(gaussian_logx(peak_co2, *_popt))
    fig.update_layout(
        **PLOT_DEFAULTS,
        height=580, width=960,
        margin=dict(l=70, r=210, t=70, b=70),
        legend=dict(title=dict(text="<b>Region</b>", font=dict(size=12)),
                    x=1.01, y=1, bgcolor="rgba(255,255,255,0.9)",
                    bordercolor="#E0E0E0", borderwidth=1),
        annotations=[
            dict(text=f"<b>Pearson r = {pearson_r:.2f}</b><br>Gaussian R² = {r2_gauss:.2f}",
            ...
            dict(text=f"Peak ≈ {peak_co2:.1f} t/person",
                 x=np.log10(peak_co2), y=peak_y + 0.25,
...
GEO_BASE = dict(showcoastlines=True, coastlinecolor="#C0C0C0",
                showland=True, landcolor="#F7F7F7",
                showocean=True, oceancolor="#E6F2F8",
                showframe=False, projection_type="natural earth", showlakes=False)
...
def _get_year_region_df(yr, region="World"):
    ...
    return (sub.groupby("iso_code", as_index=False)
...
def build_fig2(year=2022, region="World"):
    ...
    fig   = make_subplots(
        ...
        subplot_titles=[
            ...
            f"<b>CO₂ per Capita</b>  (log scale) — {region}, {year}",
        ],
        horizontal_spacing=0.04,
    )
    fig.add_trace(go.Choropleth(
        locations=yd["iso_code"], z=yd["life_satisfaction"], text=yd["country_name"],
        colorscale="RdYlGn", zmin=1.0, zmid=5.5, zmax=8.5,
        colorbar=dict(title="Score", x=0.455, len=0.72, thickness=14,
    ...
    fig.add_trace(go.Choropleth(
        locations=yd["iso_code"], z=yd["co2_log"], text=yd["country_name"],
        customdata=yd["co2_per_capita"], colorscale="Blues", zmin=0, zmax=CO2_MAX_LOG,
...
FEATURES = [
    ...
    "Explained by: Generosity", "Explained by: Perceptions of corruption", "co2_per_capita",
]
FEATURE_LABELS = [
    "Log GDP per capita", "Social support", "Healthy life expectancy",
    "Freedom of choice", "Generosity", "Perceptions of corruption", "CO₂ per capita",
]
...
r2_cub3  = _fit_poly(3, X_co2_, y_all)
...
bar_labels_left   = ["Linear (CO₂ only)", "Quadratic (CO₂ only)",
                     "Cubic (CO₂ only)", "Multiple Regression"]
...
def build_fig3():
    ...
    for label, val, color in zip(bar_labels_left,
                                 [r2_lin, r2_quad, r2_cub3, r2_multi],
                                 bar_colors_left):
        fig.add_trace(
            go.Bar(name=label, x=[label], y=[val], marker_color=color,
                   marker_line=dict(color="white", width=1.5),
                   text=[f"{val:.2f}"], textposition="outside",
                   textfont=dict(size=12, color=SLATE), width=0.55, legendgroup=label,
                   hovertemplate=f"<b>{label}</b><br>R² = {val:.3f}<extra></extra>"),
            row=1, col=1,
        )
    fig.add_hline(y=r2_multi, line_dash="dash", line_color=CORAL, line_width=1.8, row=1, col=1)
    fig.add_annotation(xref="x", yref="y", x=1.5, y=r2_multi + 0.05,
                       text=f"Multiple R² = {r2_multi:.2f}", showarrow=False,
    ...
    for i, (label, c, lo, hi, color) in enumerate(
        ...
        fig.add_trace(
            go.Bar(name=f"feat_{i}", x=[c], y=[label], orientation="h",
    ...
    fig.update_yaxes(showgrid=True, gridcolor="#F0F0F0", range=[0, 1.12],
                     tickfont=dict(size=11), title="R²", row=1, col=1)
    fig.update_xaxes(showgrid=True, gridcolor="#F0F0F0", zeroline=False,
                     tickfont=dict(size=10), title="Standardised β",
    ...
    fig.update_layout(
        **PLOT_DEFAULTS,
        title=dict(text="<b>How Much Does CO₂ Independently Explain Life Satisfaction?</b>",
                   font=dict(size=15)),
        height=540, width=1120, margin=dict(l=60, r=80, t=100, b=60),
        barmode="overlay",
        legend=dict(title=dict(text="<b>Model</b>", font=dict(size=11)),
                    x=1.01, y=1, bgcolor="rgba(255,255,255,0.9)",
                    bordercolor="#E0E0E0", borderwidth=1),
    )
    return fig
...
def build_fig4(min_pct=0):
    ...
    fig = px.scatter(
        ...
        hover_name="country",
        hover_data={"co2_per_capita": ":.2f", "life_satisfaction": ":.2f",
                    "efficiency": ":.3f", "residual_pct": ":.0f"},
        labels={"co2_per_capita": "CO₂ per Capita (tonnes/person, log scale)",
                "efficiency": "Efficiency (Life Satisfaction ÷ CO₂)"},
        title=(f"<b>CO₂ Efficiency vs. Life Satisfaction</b>  (2022)"
               f"<br><sup>{len(filtered)} of {len(df_2022)} countries "
               f"(≥ {min_pct:.0f}th efficiency percentile)</sup>"),
        log_x=True, height=580,
    )
    fig.update_traces(marker=dict(size=10, opacity=0.8, line=dict(width=0.6, color="white")))
    fig.add_trace(go.Scatter(x=x_fit_eff, y=y_fit_eff, mode="lines",
...
dash_app.layout = html.Div([
    ...
    dcc.Tabs(id="tabs", value="v1", style={"borderBottom": "2px solid #E8E8E8"}, children=[
        ...
        dcc.Tab(label="4 · CO₂ Efficiency",      value="v4", style=_tab, selected_style=_tsel),
...
def render_tab(tab):
    if tab == "v1":
        return html.Div([
            html.P("Animated scatter with 2D Gaussian fit in log(x) space, "
                   "capturing the inverted-U / diminishing-returns relationship.", style=_desc),
            dcc.Graph(figure=build_fig1(), config={"displayModeBar": True},
                      style={"height": "600px"}),
        ])
    if tab == "v2":
        return html.Div([
            html.P("Region-filtered dual choropleth. Select a region to zoom; "
                   "drag the year slider to move through 2014–2024.", style=_desc),
            html.Div([
                html.Div([
                    html.Label("Region:", style={"fontFamily": "Arial", "fontSize": "12px",
                                                 "color": SLATE, "marginRight": "8px"}),
                    dcc.Dropdown(id="v2-region",
                ...
                html.Div([
                    html.Label("Year:", style={"fontFamily": "Arial", "fontSize": "12px",
                                               "color": SLATE, "marginRight": "8px"}),
                    dcc.Slider(id="v2-year", min=YEARS_ALL[0], max=YEARS_ALL[-1],
                               step=1, value=2022,
    ...
    if tab == "v3":
        return html.Div([
            html.P("Click any coefficient bar (right panel) to toggle that feature. "
                   "The Multiple Regression R² bar and reference line update live.", style=_desc),
    ...
    if tab == "v4":
        return html.Div([
            html.P("Efficiency = Life Satisfaction ÷ CO₂ (2022). "
                   "Points above the dashed line over-perform expected efficiency. "
...
def toggle_feature(click_data, current_fig, mask_json):
    ...
    new_r2 = (r2_score(y_all, LinearRegression()
